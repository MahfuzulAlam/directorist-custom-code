...
$listing_category_ids     = $listing_form->add_listing_term_ids( ATBDP_CATEGORY );
$category_ids_for_options = $listing_category_ids;

if ( empty( $current_terms ) && is_user_logged_in() ) {
    $default_raw = get_user_meta( get_current_user_id(), 'default_categories', true );
    ...
    if ( ! empty( $default_terms ) ) {
        ...
        $category_ids_for_options = array_map(
...
<div class="directorist-form-group directorist-form-categories-field"<?php echo $conditional_logic_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Already escaped in get_conditional_logic_attributes() ?>>
    ...
    <select name="admin_category_select[]" id="at_biz_dir-categories" class="directorist-form-element" data-selected-id="<?php echo esc_attr( $current_ids_as_string ) ?>" data-selected-label="<?php echo esc_attr( $current_labels_as_string ) ?>" data-placeholder="<?php echo esc_attr( $placeholder ); ?>" data-max="<?php echo esc_attr( $data_max ); ?>" data-allow_new="<?php echo esc_attr( $data_new ); ?>" <?php echo esc_attr( $multiple ); ?> <?php $listing_form->required( $data ); ?>>
        ...
        if ( ! $lazy_load ) {
            ...
            $plan_cat = function_exists( 'is_fee_manager_active' ) && is_fee_manager_active() ? is_plan_allowed_category( $fm_plan ) : array();
            $query_args = array(
                'parent'             => 0,
                'term_id'            => 0,
                'hide_empty'         => 0,
            ...
            $categories_field = add_listing_category_location_filter( $listing_form->get_current_listing_type(), $query_args, ATBDP_CATEGORY, $category_ids_for_options, '', $plan_cat );
            echo directorist_kses( $categories_field, 'form_input' );
        }
        ?>
