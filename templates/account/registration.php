...
$user_type_author = get_directorist_option( 'user_type_author', '' );
$user_type_user = get_directorist_option( 'user_type_user', '' );
					...
					<form action="<?php the_permalink(); ?>" method="post">
						...
						<?php if ( ! empty( get_directorist_option( 'display_user_type' ) ) ) {
							...
							<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
								<input id="author_type" type="radio" name="user_type" value="author" <?php echo esc_attr( $author_checked ); ?>><label for="author_type" class="directorist-radio__label"><?php if( !empty( $user_type_author ) ) : echo $user_type_author; else: esc_html_e( 'I am an author', 'directorist' ); endif; ?>
							</div>
							<?php } ?>
							<?php if ( empty( $user_type ) || 'general' == $user_type ) { ?>
							<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
								<input id="general_type" type="radio" name="user_type" value="general" <?php echo esc_attr( $general_checked ); ?>><label for="general_type" class="directorist-radio__label"><?php if( !empty( $user_type_user ) ) : echo $user_type_user; else: esc_html_e( 'I am a user', 'directorist' ); endif; ?>
							</div>
							<?php } ?>
						<?php } ?>
