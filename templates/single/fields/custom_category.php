<?php
/**
 * Custom Category field for Directorist single listing.
 *
 * Custom taxonomy: ATBDP_CATEGORY
 *
...
$label      = ( isset( $data['label'] ) && ! empty( $data['label'] ) ) ? $data['label'] : __( 'Category', 'directorist' );
$listing_id = ( isset( $data['listing_id'] ) && ! empty( $data['listing_id'] ) ) ? absint( $data['listing_id'] ) : get_the_ID();

$terms = array();
if ( $listing_id ) {
	$terms = wp_get_post_terms( $listing_id, ATBDP_CATEGORY );
}
